...
@synthesize isOverlayShow = _isOverlayShow;
...
- (void)setProgress:(float)progress {
    ...
    [self setMessage:[NSString stringWithFormat:@"上传进度:%.1f%%",progress * 100] animated:NO];
    if(progress >= 1)
    {
        [self setMessage:@"上传完成" animated:NO];
        [self performSelector:@selector(dismissAnimated:) withObject:[NSNumber numberWithBool:YES] afterDelay:1];
    }
}
...
- (void)setStatusBarStyle:(UIStatusBarStyle)statusBarStyle {
    if (UIStatusBarStyleBlackOpaque == statusBarStyle ||
        ...
        [self setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"status-bar-pattern-default@2x.jpg"]]];
...
- (void)animatateview:(UIView *)view
    ...
    } else if (BWStatusBarOverlayAnimationTypeFromTop == animationType)
    {
        ...
        {
            rect = CGRectMake(0, 0, LCDH, 20);
        }
        else
        {
            rect = CGRectMake(0, 0, LCDW, 20);
        }
        [view setFrame:rect];
